...
MyOpenGLWidget::MyOpenGLWidget(QWidget* parent) : QOpenGLWidget(parent) {
    data->polygons = NULL;
    data->array_for_vertices = NULL;
    data->array_for_facets = NULL;
    ...
    settings = new QSettings(QCoreApplication::applicationDirPath() + "/my_settings.conf", QSettings::IniFormat);
}
...
void MyOpenGLWidget::read_settings() {
    ...
    background_color = settings->value("Background color", background_color).value<QColor>();
...
void MyOpenGLWidget::paintGL() {
    ...
    glClearColor(background_color.red() / 255.0, background_color.green() / 255.0, background_color.blue() / 255.0, 0);
...
void MyOpenGLWidget::drawObject() {
    glColor3d(vertex_color.red() / 255.0, vertex_color.green() / 255.0, vertex_color.blue() / 255.0);
    if (vertex_type == 1) {
    ...
    glVertexPointer(3, GL_DOUBLE, 0, data->array_for_vertices);
    glEnableClientState(GL_VERTEX_ARRAY);
    if (vertex_type != 0) {
        glDrawArrays(GL_POINTS, 1, data->count_of_vertices);
    }
    glColor3d(facet_color.red() /255.0, facet_color.green() / 255.0, facet_color.blue() / 255.0);
    glLineWidth(thickness_size);
    glLineStipple(2, 0X00FF);
    if (edge_type == 1) {
        glEnable(GL_LINE_STIPPLE);
    }
    glDrawElements(GL_LINES, data->count_v * 2, GL_UNSIGNED_INT, data->array_for_facets);
